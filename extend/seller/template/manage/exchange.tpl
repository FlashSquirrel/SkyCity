		...
		<span class="s4 fl"><?=date('y/m/d H:i',$value['addtime']);?></span>
	 	...
	 	<span class="s5 fl">
	 		<?php if(!($value['status']==1 && $value['end']>$_timestamp)){ ?>
		 		...
		 		<a href="<?=u(MODNAME,'callback',array('op'=>'appaly','type'=>$op,'id'=>$value['exchange_id']));?>">立即付款</a>
		 		<?php } ?>
		  		<a href="<?=u(MODNAME,'deals',array('type'=>$op,'id'=>$value['exchange_id']));?>">编辑</a>
				<a href="<?=u(MODNAME,'del',array('type'=>$op,'id'=>$value['exchange_id']));?>">删除</a>
			<?php } ?>
		</span>
	</div>
	<?php if($value['status']==-1){ ?>
