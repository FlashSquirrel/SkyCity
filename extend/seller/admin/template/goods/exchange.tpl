...
<ul class="nav">
	<li <?=$active['0'];?>><a href="<?=$_extend_url;?>&pmod=apply&op=exchange&status=0">待审核兑换</a></li>
	<li <?=$active['-1'];?>><a href="<?=$_extend_url;?>&pmod=apply&op=exchange&status=-1">拒绝的兑换</a></li>
	<li <?=$active['1'];?>><a href="<?=$_extend_url;?>&pmod=apply&op=exchange&status=1">已审核通过</a></li>
</ul>
<div class="box-content">
	<?php if($do=='edit'){ ?>
	<?php include(PATH_TPL."/main/exc/add.tpl.php");?>
	<?php }else{ ?>
	<div class="table">
      	<form method="GET">
      	<div class="th">
    ...
    <div class="table">
        ...
        <?php foreach ($exclist as $key=>$value){ ?>
        <tr id="data_<?=$value['exchange_id'];?>">
        	<td class="text-center">
        		<input type="checkbox" name="id[]" value="<?=$value['exchange_id'];?>" class="checkbox" onclick="checkoption($('.allbox'));">
        	</td>
        	<td class="text-center">
        	...
        	<td class="text-center">
            	...
            	[<a href="<?=$_extend_url;?>&pmod=apply&op=exchange&do=edit&status=<?=$status;?>&id=<?=$value['exchange_id'];?>">修改</a>]&nbsp;
            	<?php }else{ ?>

	            	<?php if($value['status']==0){ ?>
	            	[<a href="javascript:;" onclick="refuse('<?=$value['exchange_id'];?>','exchange');">拒绝</a>]&nbsp;
	            	<?php } ?>
	         		[<a href="javascript:;" onclick="audit('<?=$value['exchange_id'];?>','exchange')">通过排期</a>]&nbsp;
         		<?php } ?>
            </td>
        </tr>
        <?php if($value['status']==-1){ ?>
